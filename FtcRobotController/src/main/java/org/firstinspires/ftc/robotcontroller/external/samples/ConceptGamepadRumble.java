...
{
    boolean lastA;                      // Use to track the prior button state.
    boolean lastLB;                     // Use to track the prior button state.
    boolean highLevel;                  // used to prevent multiple level-based rumbles.
    boolean secondHalf;                 // Use to prevent multiple half-time warning rumbles.
    ...
    public void runOpMode()
    {
	    ...
	    this.customRumbleEffect = new Gamepad.RumbleEffect.Builder()
	    ...
	    this.waitForStart();
	    this.runtime.reset();    // Start game timer.

        // Loop while monitoring buttons for rumble triggers
        while (this.opModeIsActive())
        {
            // Read and save the current gamepad button states.
            final boolean currentA  = this.gamepad1.a ;
            final boolean currentLB = this.gamepad1.left_bumper ;

            // Display the current Rumble status.  Just for interest.
	        this.telemetry.addData(">", "Are we RUMBLING? %s\n", this.gamepad1.isRumbling() ? "YES" : "no" );
            ...
            // ----------------------------------------------------------------------------------------
            if ((HALF_TIME < runtime.seconds()) && ! this.secondHalf)  {
	            this.gamepad1.runRumbleEffect(this.customRumbleEffect);
	            this.secondHalf =true;
            }

            // Display the time remaining while we are still counting down.
            if (! this.secondHalf) {
	            this.telemetry.addData(">", "Halftime Alert Countdown: %3.0f Sec \n", (this.HALF_TIME - this.runtime.seconds()) );
            }
            ...
            if (currentLB) {
	            ...
	            this.gamepad1.rumble(this.gamepad1.left_trigger, this.gamepad1.right_trigger, Gamepad.RUMBLE_DURATION_CONTINUOUS);

                // Show what is being sent to rumbles
	            this.telemetry.addData(">", "Squeeze triggers to control rumbles");
	            this.telemetry.addData("> : Rumble", "Left: %.0f%%   Right: %.0f%%", this.gamepad1.left_trigger * 100, this.gamepad1.right_trigger * 100);
            } else {
                // Make sure rumble is turned off when Left Bumper is released (only one time each press)
                if (this.lastLB) {
	                this.gamepad1.stopRumble();
                }

                //  Prompt for manual rumble action
	            this.telemetry.addData(">", "Hold Left-Bumper to test Manual Rumble");
	            this.telemetry.addData(">", "Press A (Cross) for three blips");
	            this.telemetry.addData(">", "Squeeze right trigger slowly for 1 blip");
            }
	        this.lastLB = currentLB; // remember the current button state for next time around the loop
            ...
            // ----------------------------------------------------------------------------------------
            if (currentA && ! this.lastA) {
                if (! this.gamepad1.isRumbling())  // Check for possible overlap of rumbles.
	                this.gamepad1.rumbleBlips(3);
            }
	        this.lastA = currentA; // remember the current button state for next time around the loop


            // ----------------------------------------------------------------------------------------
            // Example 4. Rumble once when gamepad right trigger goes above the THRESHOLD.
            // ----------------------------------------------------------------------------------------
            if (TRIGGER_THRESHOLD < gamepad1.right_trigger) {
                if (! this.highLevel) {
	                this.gamepad1.rumble(0.9, 0, 200);  // 200 mSec burst on left motor.
	                this.highLevel = true;  // Hold off any more triggers
                }
            } else {
	            this.highLevel = false;  // We can trigger again now.
            }
